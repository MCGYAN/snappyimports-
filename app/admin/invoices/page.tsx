    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">
          Create a one-off invoice for any customer. History is permanent and cannot be deleted. Each
          invoice records the team account that generated it.
        </p>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]">
        ...
        <section className="space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            {selected?.createdBy?.label ? (
              <p className="w-full text-xs text-slate-500 sm:w-auto">
                Generated by {selected.createdBy.label}
                {selected.createdBy.email && selected.createdBy.fullName
              ...
              <tbody>
                {invoices.map((invoice) => (
                  <tr key={invoice.id} className="border-b border-slate-100">
                    ...
                    <td className="py-2.5 pr-3">
                      <div className="font-medium text-slate-800">
                        {invoice.createdBy?.label ||
                          String(invoice.data?.created_by_name || invoice.data?.created_by_email || '—')}
                      </div>
                      {invoice.createdBy?.email && invoice.createdBy?.fullName ? (
                        <div className="text-xs text-slate-500">{invoice.createdBy.email}</div>
                      ) : null}
                    </td>
