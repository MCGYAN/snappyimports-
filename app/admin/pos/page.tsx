...
export default function POSPage() {
    ...
    const regionOptions = [
        'Metro', 'North', 'South', 'East', 'West', 'Central',
        'North East', 'North West', 'South East', 'South West',
        'Rural (Zone A)', 'Rural (Zone B)', 'Other'
    ];
    ...
    const handleCheckout = async () => {
        ...
        try {
            const orderNumber = `ORD-GH¢{Date.now()}-GH¢{Math.floor(Math.random() * 1000)}`;
            ...
            const posRes = await fetch('/api/admin/pos', {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    paymentStatus: isCashOrCard ? 'paid' : 'pending',
                    ...
                    total: grandTotal,
                    addressData,
                    cart: cart.map(item => ({ id: item.id, name: item.name, cartQuantity: item.cartQuantity, price: item.price, image: item.image })),
                    isCashOrCard
                })
            });
            ...
            if (!posRes.ok || !posData.success) {
                throw new Error(posData.error || 'Failed to create POS order');
            }

            const order = posData.order;
                ...
                <div className="bg-white p-4 border-b border-gray-200 flex items-center justify-between space-x-4 shrink-0">
                    <div className="relative flex-1 max-w-lg">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-accent/25 text-sm"
                            autoFocus
                        />
                    </div>
                                ...
                                >
                                    ...
                                    <div className="p-3 flex flex-col flex-1">
                                        ...
                                        <div className="flex items-center justify-between mt-2 pt-2">
                                            <span className="text-brand-primary font-bold">GH¢{product.price.toFixed(2)}</span>
                                            <button className="w-8 h-8 rounded-full bg-brand-primary/5 text-brand-primary flex items-center justify-center group-hover:bg-brand-accent group-hover:text-white transition-colors">
                ...
                {cart.length > 0 && (
                    <div className="lg:hidden p-4 border-t border-gray-200 bg-white fixed bottom-0 left-0 right-0 z-30 shadow-2xl safe-area-bottom">
                        <button
                            ...
                            className="w-full py-3 bg-brand-primary text-white rounded-xl font-bold flex justify-between px-6 shadow-lg active:scale-95 transition-transform"
                        >
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                    {cart.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-gray-400 space-y-4">
                            ...
                            <button onClick={() => setIsMobileCartOpen(false)} className="lg:hidden text-brand-primary font-medium hover:underline">
                ...
                <div className="p-4 bg-gray-50 border-t border-gray-200 space-y-4 shrink-0 safe-area-bottom">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        ...
                        <button
                            ...
                            disabled={cart.length === 0}
                            className="px-4 py-3 bg-brand-primary text-white rounded-lg hover:bg-brand-accent font-bold text-sm shadow-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                    ...
                    <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                        {completedOrder ? (
                            ...
                            <div className="p-8 text-center flex flex-col items-center justify-center space-y-6 overflow-y-auto">
                                <div className={`w-20 h-20 rounded-full flex items-center justify-center ${completedOrder.paymentPending ? 'bg-amber-100' : 'bg-brand-primary/10'}`}>
                                    <i className={`text-5xl ${completedOrder.paymentPending ? 'ri-time-line text-amber-600' : 'ri-checkbox-circle-fill text-brand-primary'}`}></i>
                                </div>
                                <div>
                                    ...
                                    {!completedOrder.paymentPending && paymentMethod === 'cash' && changeDue > 0 && (
                                        <div className="mt-3 bg-brand-primary/5 border border-brand-primary/20 rounded-lg p-3">
                                            <p className="text-sm text-brand-primary">Change Due</p>
                                            <p className="text-2xl font-bold text-brand-primary">GH¢{changeDue.toFixed(2)}</p>
                                        </div>
                                    )}
                                        ...
                                        <div className="mt-4 space-y-3">
                                            ...
                                            <div className="mt-2">
                                                <button
                                                    ...
                                                    className="text-sm text-brand-primary hover:text-brand-accent font-medium underline"
                                                >
                                ...
                                <div className="grid grid-cols-2 gap-4 w-full mt-4">
                                    ...
                                    <button onClick={resetCheckout} className="py-3 px-4 bg-brand-primary text-white rounded-xl font-semibold hover:bg-brand-accent transition-colors">
                                        New Order
                                    </button>
                                </div>
                            </div>
                            ...
                            <>
                                ...
                                <div className="p-6 space-y-6 overflow-y-auto">
                                    ...
                                    {/* Total Display */}
                                    <div className="text-center py-4 bg-brand-primary/5 rounded-xl border border-brand-primary/15">
                                        <p className="text-sm text-brand-primary uppercase tracking-wide font-semibold">Amount to Pay</p>
                                        <p className="text-4xl font-extrabold text-brand-primary mt-1">GH¢{grandTotal.toFixed(2)}</p>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="relative mb-2">
                                            ...
                                            <input
                                                ...
                                                className="w-full pl-9 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-accent/25 outline-none text-sm"
                                            />
                                        </div>

                                        <select
                                            className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-accent/25 outline-none mb-2"
                                            onChange={(e) => {
                                        ...
                                        {selectedCustomer && (
                                            <div className="bg-brand-primary/5 border border-brand-primary/20 rounded-lg p-3 mb-2 flex items-center justify-between">
                                                <div>
                                            ...
                                            <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 mt-2 space-y-3">
                                                ...
                                                <div className="grid grid-cols-2 gap-3">
                                                    <input
                                                        ...
                                                        className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-brand-accent/25 text-sm"
                                                    />
                                                    <input
                                                        type="text"
                                                ...
                                                <div className="grid grid-cols-2 gap-3">
                                                    ...
                                                    <input
                                                        ...
                                                        className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-brand-accent/25 text-sm ${paymentMethod === 'momo' && !guestDetails.phone ? 'border-amber-400 bg-amber-50' : 'border-gray-300'
                                                            }`}
                                                    />
                                                </div>
                                            </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            ...
                                            >
                                                <i className={`ri-store-2-line text-xl ${deliveryMethod === 'pickup' ? 'text-brand-primary' : 'text-gray-400'}`}></i>
                                                <div className="text-left">
                                                    <p className={`text-sm font-semibold ${deliveryMethod === 'pickup' ? 'text-brand-primary' : 'text-gray-700'}`}>Store Pickup</p>
                                            ...
                                            >
                                                <i className={`ri-truck-line text-xl ${deliveryMethod === 'doorstep' ? 'text-brand-primary' : 'text-gray-400'}`}></i>
                                                <div className="text-left">
                                                    <p className={`text-sm font-semibold ${deliveryMethod === 'doorstep' ? 'text-brand-primary' : 'text-gray-700'}`}>Doorstep Delivery</p>
                                        ...
                                        {deliveryMethod === 'doorstep' && (
                                            <div className="mt-3 bg-brand-primary/5 p-4 rounded-lg border border-brand-primary/20 space-y-3">
                                                <h4 className="text-sm font-bold text-gray-900 flex items-center">
                                                    <i className="ri-map-pin-line mr-2 text-brand-primary"></i>
                                                    Delivery Address
                                                </h4>
                                                <input
                                                    type="text"
                                            ...
                                            {[
                                                ...
                                                <button
                                                    ...
                                                    className={`py-3 rounded-lg font-medium border transition-all flex flex-col items-center space-y-1 ${paymentMethod === method.key
                                                        ? 'border-brand-accent bg-brand-primary/5 text-brand-primary ring-1 ring-brand-accent'
                                    ...
                                    {paymentMethod === 'cash' && (
                                        <div>
                                            ...
                                            {changeDue > 0 && (
                                                <p className="text-right text-brand-primary font-bold mt-2">Change: GH¢{changeDue.toFixed(2)}</p>
                                            )}
                                    ...
                                    {paymentMethod === 'card' && (
                                        <div className="bg-brand-primary/5 border border-brand-primary/20 rounded-lg p-3">
                                            <div className="flex items-start space-x-2">
                                                <i className="ri-bank-card-line text-brand-primary mt-0.5"></i>
                                ...
                                <div className="p-6 border-t border-gray-100 bg-gray-50 shrink-0">
                                    <button
                                        ...
                                        disabled={processing}
                                        className="w-full py-4 bg-brand-primary text-white rounded-xl font-bold text-lg shadow-lg hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center space-x-2"
                                    >
                                        {processing ? (
                                            <>
