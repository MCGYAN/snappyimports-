...
function ShopContent({
  ...
  const [products, setProducts] = useState(() => formatShopProducts(initialProducts));
  const [categories, setCategories] = useState<any[]>(
    initialCategories.length
      ? [{ id: 'all', name: 'All imports', count: 0 }, ...initialCategories]
      : [{ id: 'all', name: 'All imports', count: 0 }],
  );
  const [loading, setLoading] = useState(false);
  const [totalProducts, setTotalProducts] = useState(initialCount);
  ...
  useEffect(() => {
    ...
    async function fetchProducts() {
      ...
      try {
        ...
        if (data) {
          setProducts(formatShopProducts(data));
    ...
    void fetchProducts();
  }, [selectedCategory, priceRange, selectedRating, sortBy, page, searchParams, categories, initialSearch]);
